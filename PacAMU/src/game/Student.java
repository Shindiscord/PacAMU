...
class Student  extends MovableObject implements amuGameObject, KeyboardListener{
	...
	private final double basePosX;
	private final double basePosY;
	...
	private final SingleSprite leftSprite = new SingleSprite(
			new Image("file:res/img/Player/male_l_30_51_8.png")
			,30, 51, 8);
	private final SingleSprite upSprite = new SingleSprite(
			new Image("file:res/img/Player/male_u_30_51_8.png")
			,30, 51, 8);
	
	private final SingleSprite rightSprite = new SingleSprite(
			new Image("file:res/img/Player/male_r_30_51_8.png")
			,30, 51, 8);
	private final SingleSprite downSprite = new SingleSprite(
			new Image("file:res/img/Player/male_d_30_51_8.png")
			,30, 51, 8);
	
	ChangeableSprite s;
	private final int bordureH, bordureV;
	
	public Sprite getSprite() {
		return this.s;
	}
	
	Student(final double x, final double y, final int bordureH, final int bordureV, final Grid map){
	...
	public void onKeyReleased(final KeyCode key) {

	}
	
	public void update(final long msSinceLastCall){
		
		final double currentGridX = Math.floor(this.getX()/this.gridSize);
		final double currentGridY = Math.floor(this.getY()/this.gridSize);
	...
	public int get_lives(){
		return this.lifeManager.get_lives();
	}

	public void death(){
		this.lifeManager.remove_life();
		this.setPos(this.basePosX, this.basePosY);
		this.currentDirection = KeyCode.RIGHT;
		this.nextDirection = KeyCode.RIGHT;
		this.setHspeed(4);
		this.setVspeed(0);
		this.s.switchTo(rightSprite);
		System.out.println(lifeManager.get_lives());
	}
