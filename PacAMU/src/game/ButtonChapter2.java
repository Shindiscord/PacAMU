...
public class ButtonChapter2 extends Clickable{

	
	
	public ButtonChapter2(String name) {
		super(name);
		this.button.setMaxSize(220, 50);
	}
	
	public ButtonChapter2(Image img) {
		super(img);
		this.button.setMaxSize(220, 50);
	}
	...
	public void onClick() {
		GameRoom chapter2 = new GameRoom(40);
        
    	Grid grid = null;
		try {
			grid = new Grid("res/mapFiles/map2.txt", 20, 14);
		} catch (IOException e) {
        ...
        TileManager.placeTiles(chapter2, grid, 32, 32);
        TileManager.placeCollectables(chapter2, grid, 32, 32);
        Student stud = null;
        ...
        for( int i=0 ; i<grid.getLargeur() ; i++) {
        	for( int j=0 ; j<grid.getHauteur() ; j++) {
        		if(grid.getTile(i, j) == '3') {
        	        chapter2.addObject(new BoarLvl2(32*i, 32*j, 32*19, 32*13,grid ,1, stud));
        		}
        		else if (grid.getTile(i, j) == '2'){
        	        chapter2.addObject(new BoarLvl1(32*i, 32*j, 32*19, 32*13,grid ,1));
        		}
        	}
        }
        ...
        back.setPos(400, 400);
        */
        GameManager.changeRoom(chapter2);
